const ApiKey = "6ae9522fdc5f4ef3a2a4278960311b5d";
...
function getDetailItem( id ) {
    ...
    let teamDetails = baseUrl + 'teams/' + id;
    fetch( teamDetails, fetchHeader )
        .then(response => response.json())
        .then(data => {
            ...
            title.innerHTML = "Detail Tim " + data.name;
            data.squad.forEach( team => {

                teams += `<tr>
                    <td><b>${team.name}</b><br>Position : ${team.position}</td>
                    <td>Nomor : ${team.shirtNumber}<br>Kewarganegaraan : ${team.nationality}</td>
                </tr>`;
            } );
